...
export function CloverIcon({ className, size = "md", animated = false, color }: CloverIconProps) {
  return (
    <div className={cn(sizeClasses[size], animated && "animate-bounce-slow", className)}>
      <svg viewBox="0 0 100 100" className="h-full w-full drop-shadow-lg">
        ...
        <g fill={color ?? "url(#mainCloverGradient)"}>
          ...
          <rect x="47" y="65" width="6" height="28" rx="3" fill={color ?? "#16a34a"} />
        </g>
      </svg>
    </div>
  );
